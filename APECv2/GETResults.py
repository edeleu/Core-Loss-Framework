...
#Training Materials 
materials = {
    "N87" : ["/scratch/gpfs/sw0123/N87/N87_R34.0X20.5X12.5_Data5_Seq2Scalar_Downsampled_FullFixed.json", 0, 490, 900,2200]
}

#materials: material name : [filepath,ID, Bsat, freqPeak, mu_i]
# transferMaterial = {
#     "3E6" : ["/scratch/gpfs/sw0123/3E6_TL/3E6_TX-22-14-6.4_Data1_Seq2Scalar_Downsampled_Full.json", 11, 390,50,12000]
# }

transferMaterial = {
    "78" : ["/scratch/gpfs/sw0123/78_TL/78_0076_Data1_Seq2Scalar_Downsampled_Full.json", 11, 480,900,2300]
}
...
# #Testing Materials
# materials = {
#     "N87" : ["/scratch/gpfs/sw0123/N87/N87_R34.0X20.5X12.5_Data5_Seq2Scalar_Downsampled_FullTest.json", 0]
# }
...
def transferLearn():
    ...
    state_dict = torch.load('/scratch/gpfs/sw0123/GeneralFramework466.sd')
    ...
    for DATA_SIZE in range(100,3500,100):
        #ENFORCE REPRODUCIBILITY OF TRIALS OUTSIDE THIS ENV!
        #otherwise, trials will be reproducible in this setup, but not outside of it
        #consec trials randomness according to seed, but different randomness!
        random.seed(374327224)
        np.random.seed(374327224)
        torch.manual_seed(374327224) #OLD: 92323
        torch.backends.cudnn.deterministic = True
        ...
        Tstate_dict = torch.load('/scratch/gpfs/sw0123/ExperimentalHead0.sd')
...
#        transferNetwork = loadMaterialNetwork(1, numFeatures)
        ...
        for epoch_i in range(TRANSFER_EPOCH):
            ...
            if (epoch_i == 20):
                ...
                o3Best = o3
                o4Best = o4
            elif (epoch_i > TRANSFER_EPOCH-140):
                if (o1<o1Best):
                    o1Best = o1
                    o2Best = o2
                    o3Best = o3
    ...
    with open("Mat77_NOTL_EarlyStop.csv", "w") as f:
        writer = csv.writer(f)
...
def loadMaterialNetwork(x, numFeatures):
    ...
    state_dict = torch.load('/scratch/gpfs/sw0123/UnifiedTransformer623.sd')
...
def avgMatNetworks():
    ...
    sd0 = torch.load('/scratch/gpfs/sw0123/ExperimentalHead0.sd')
    sd1 = torch.load('/scratch/gpfs/sw0123/ExperimentalHead1.sd')
    sd2 = torch.load('/scratch/gpfs/sw0123/ExperimentalHead2.sd')
    sd3 = torch.load('/scratch/gpfs/sw0123/ExperimentalHead3.sd')
    sd4 = torch.load('/scratch/gpfs/sw0123/ExperimentalHead4.sd')
    sd5 = torch.load('/scratch/gpfs/sw0123/ExperimentalHead5.sd')
    ...
    torch.save(sd2, "/scratch/gpfs/sw0123/AVGNetwork.sd")
...
def limitedTL(trial):
    ...
    TLRINI = trial.suggest_float("LR", 0.0008, 0.06)
    TLRMin = trial.suggest_float("MIN_LR", 0.0001, 0.00075)

    TrEpoch = trial.suggest_int("epochs", 140, 300)
    TBatch = trial.suggest_int("batch", 56, 264, 8)
...
#ExperimentalHead6.sd is AVG
    ...
    transferNetwork = loadMaterialNetwork(1, numFeatures)
    Tstate_dict = torch.load('/scratch/gpfs/sw0123/ExperimentalHead' + str(listIndex) + '.sd')
    transferNetwork.load_state_dict(Tstate_dict, strict=False)
...
def optimizeMe():
    study_name = "RetryLimited3E6TL"  # Unique identifier of the study.
    ...
    study.optimize(limitedTL, n_trials=150, gc_after_trial=True)
...
if __name__ == "__main__":
    #optimizeMe()
    # for i in range(0,6):
    #     existingInference(torch.load('/scratch/gpfs/sw0123/ExperimentalHead' + str(i) +'.sd'))
    #avgMatNetworks()
    transferLearn()
