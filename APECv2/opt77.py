    ...
    for DATA_SIZE in range(100,5000,100):
        ...
        for epoch_i in range(TRANSFER_EPOCH):
            ...
            for in_B, freqTensor, parameterTensor, labels, out in trainData:
                ...
                loss = criterion(output.to(device), out.to(device))
                loss.backward()
...
def avgMatNetworks():
    ...
    sd1 = torch.load('/scratch/gpfs/ed5754/UnifiedTransformer623.sd')
    sd2 = torch.load('/scratch/gpfs/ed5754/UnifiedTransformer623.sd')
    
    # Average all parameters
    for key in sd1:
        sd2[key] = (sd1[key] + sd2[key]) / 3.0

    # Save the averaged material model!
    torch.save(sd2, "/scratch/gpfs/ed5754/AVGMatNet.sd")
...
def singleTL(trial):
    ...
    TLRINI = trial.suggest_float("LR", 0.0009, 0.01)
    TLRMin = trial.suggest_float("MIN_LR", 0.0001, 0.00089)
...
def optimizeMe():
    study_name = "Opt78"  # Unique identifier of the study.
