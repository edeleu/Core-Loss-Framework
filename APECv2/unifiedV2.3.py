## Add re-norm terms into material specific mapping nets!
## do this by holding them as constants, addtnl inputs
...
def load_dataset(info, data_length=128):
    ...
    bSatVec  = torch.full((out.size()), bSaturation*10)
    fNormVec  = torch.full((out.size()), freqPeak/(10000))

    parameterTensor = torch.cat((bSatVec,fNormVec,parameterTensor), dim=1)
...
def main():
    ...
    for epoch_i in range(NUM_EPOCH):
        ...
        # Afterwards, save model at every epoch and record validation error as an array. 
            ...
            torch.save(net.state_dict(), "/scratch/gpfs/ed5754/UptoDatest/GeneralFrameworkSix" + str(epoch_i) + ".sd")
    ...
    with open("UpToDatests.csv", "w") as f:
        writer = csv.writer(f)
