...
class MagUniformer(nn.Module):
    def __init__(self, 
        ...
        self.heads = nn.Sequential(*[MaterialHead(output_features, id) for id in range(num_materials)])
...
def load_dataset(info, data_length=128):
    ...
    # Combine the scalars into one tensor with three columns (LEN x 3)
...
def main():
    ...
    for epoch_i in range(NUM_EPOCH):
        ...
        if (epoch_i < NUM_EPOCH-100):
            if (epoch_i+1)%2 == 0:
                with torch.no_grad():
                    net.eval()
                    epoch_valid_loss = 0
                    for in_B, in_tensors, labels, out in valid_loader:
                        output = net(in_seq=in_B.to(device), vars=in_tensors.to(device), materials=labels.to(device), device=device)
                        loss = criterion(output, out.to(device))
        ...
        # Afterwards, save model at every epoch and record validation error as an array. 
            ...
            torch.save(net.state_dict(), "/scratch/gpfs/sw0123/SmallModel/UnifiedTransformer" + str(epoch_i) + ".sd")
